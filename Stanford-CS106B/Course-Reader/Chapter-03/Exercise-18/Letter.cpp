...
string generateCipherText(const string& plaintext, const string& key);
void generateKeys(const string& key, string& upper, string& lower);

#pragma mark -
#pragma mark Definitions
/*
 * Function: generateCipherText(const string& plaintext, const string& key);
 * -------------------------------------------------------------------------
 * Generate the letter-substitution cipher text from the plaintext and key.
 */
string generateCipherText(const string& plaintext, const string& key)
{
    static string kBasis = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
int main() {
    ...
    while ( true ) {
        ...
        //
        plaintext = getLine("Enter a message: ");
